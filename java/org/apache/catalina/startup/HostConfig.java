...
public class HostConfig implements LifecycleListener {
     ...
     * in our "application root" directory. 部署应用的三种方式：1.描述符部署，2.war包部署，3.文件夹部署
     */
    protected void deployApps() {
        File appBase = host.getAppBaseFile();
        File configBase = host.getConfigBaseFile();
        String[] filteredAppPaths = filterAppPaths(appBase.list());
        // Deploy XML descriptors from configBase 描述符部署
    ...
    protected void check() {
        ...
        if (host.getAutoDeploy()) {
            ...
            DeployedApplication[] apps = deployed.values().toArray(new DeployedApplication[0]); // 检查这个 Host 下所有已经部署的 Web 应用
            for (DeployedApplication app : apps) {
                if (tryAddServiced(app.name)) {
                    try {
                        checkResources(app, false); // 检查 Web 应用目录是否有变化
                    } finally {
